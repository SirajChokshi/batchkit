...
![DevTools](/docs/devtools-1.png)

See how your requests are being batched and scheduled in real-time in a browser environment. For server-side tracing, see the [server-side tracing](/docs/guide#server-side-tracing) section in the guide.
...
```typescript
const users = batch(fetchUsers, 'id');
```

Unnamed batchers appear as `unnamed-1`, `unnamed-2`, etc. For better labels, provide a `name`:
